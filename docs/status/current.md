...
按上述顺序先推进基础对象与成员配置，不同时启动多个功能方向。9 月 10 日代码与文档已收尾，提交回顾见[当日记录](reviews/2026-09-10-daily-closeout.md)。

1. **先完成合同审查**：从现有 bootstrap、Team / Project / Channel schema 和权限 resolver 出发，明确谁能建立 owner Team、创建 Project / Channel、配置 Project 角色与受限 Channel 成员；列出审计、幂等、跨 Workspace、归档和撤权规则。涉及数据模型、权限或公共协议的新增取舍先说明影响并确认范围。
2. **再做最小纵向实现**：合同确认后，交付“新实例 owner → 基础对象配置 → 邀请成员 → 成员从首页进入可读频道并发送 Message”的受控路径；业务配置使用正式入口，禁止用手改数据库或自动提权补齐产品缺口。
3. **按切片补证据**：覆盖越权、重复请求、失败回滚和撤权后的不可发现性；补本地邀请全流程浏览器验收。当前版本 Compose 镜像、migration 008 与邮箱映射后的登录 / readiness 升级演练另按明确范围执行，不复用旧拓扑通过结论。

Radish OIDC、Document、真实 Jenkins 接入继续按近期顺序等待，不因明天清单自动启动。明天的服务启动、依赖、真实实例与远程操作仍需按根协作规则确认，本清单不沿用今天的临时环境授权。
