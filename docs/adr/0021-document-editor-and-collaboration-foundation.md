...
接受日期：2026-09-14；采用 2026-09-05 收窄的 M0.5 / M1 非 CRDT 路径。下文按日期保留评估过程，阶段 B / C 仍未启动。
...
2026-09-14 已形成 [ADR-0028：最小 Markdown Document 与不可变版本](0028-minimal-markdown-document.md)，补齐非 CRDT 路径的字段、权限、保存冲突、关系、渲染与恢复提议。项目所有者已确认 ADR-0028 与其中的固定版本 Go parser 依赖范围；阶段 A 证据及本轮无网络门禁满足下述非 CRDT 接受条件，因此本 ADR 与 ADR-0028 均改为“已接受”。正式存储 / HTTP 尚未实现，按所有者要求安排到下一工作日；正式富文本依赖与阶段 B / C 不在此次范围。
