...
`web/` 是 RadishNexus 第一正式产品形态的 React + TypeScript 入口。当前只实现 Decision Nexus View 代表原型，用来验证 Current、Relations、Timeline、权限占位与关键页面状态，不代表业务 HTTP API、完整 Web Shell、导航或聊天能力已经开放。
...
- 状态检视器只用于人工复核 data、loading、empty 与 error，不是未来产品导航。
