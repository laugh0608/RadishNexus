// Package entityref implements the canonical M0 EntityRef representation.
package entityref
...
// Registry contains the accepted entity type and ID-prefix pairs. An empty
// prefix reserves a type whose ID format has not yet been frozen.
type Registry map[string]string

// M0Registry returns a new registry with the six frozen M0 entity types.
...
func (registry Registry) Validate(ref Ref) error {
	...
	if !isCanonicalToken(ref.Type) || !isCanonicalToken(ref.ID) {
		return fmt.Errorf("%w: type and ID must be non-empty ASCII tokens", ErrInvalidReference)
	}
	if prefix != "" && !strings.HasPrefix(ref.ID, prefix) {
